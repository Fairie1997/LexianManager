...
<html>
	...
	<body>
		...
		<div class="container">
			...
			<div class="form-horizontal" id="myinfo">
	...
	<script>
	 ...
	 $('#btn_edit').click(function () {
      console.log('edit');
        ...
        $("#editMyInfo").modal();
        var info = $("#myinfo").find('input')
        var inputs = $("#editMyModal").find('input')
        $(inputs[0]).val($(info[0]).val())
        $(inputs[1]).val($(info[2]).val())
        $(inputs[2]).val($(info[3]).val())
      
    })
		
    $("#saveEdit").click(function() {
      var editMyName = $("#editMyName").val()
      var editMyPhone = ${userInfoVo.u_phone}
	...
	array.push({"editMyName":editMyName,"editMyPhone":editMyPhone,"editMySex":editMySex,"editMyEmail":editMyEmail});
      ...
      $.ajax({
          ...
          success:function(result){
        	  var info = $("#myinfo").find('input');
        	  $(info[0]).val(editMyName);
        	  $(info[1]).val(editMyPhone);
        	  $(info[2]).val(editMySex);
        	  $(info[3]).val(editMyEmail);
          },
