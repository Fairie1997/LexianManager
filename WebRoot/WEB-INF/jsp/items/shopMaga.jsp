        ...
        <div class="modal-content">
          ...
          <div id="addShopModal" class="modal-body">
            ...
            <div class="form-group">
                <label for="shopId" class="col-sm-4 control-label">门店编号:*</label>
                <div class="col-sm-10">
                  <input class="form-control" id="shopId" type="text">
                </div>
              </div>
              <div class="form-group">
    ...
    $("#addShopsInfo").on('click', function() {
      ...
      if (data.length) {
        if ($("#addShopModal").find('input').val() !== '') {
          ...
          var addShopInfos = [].concat(shopId, shopName, shopAddress, shopStates);
          for (var i = 0; i < addShopInfos.length; i++) {
            if (addShopInfos[i] === '') {
              alert(titles[i] + '内容不能为空')
            }
          }
          table.row.add([shopId ,shopName, shopAddress, shopStates]).draw();
          
            array=[];
      		array.push({"ShopId":addShopInfos[0],"ShopName":addShopInfos[1],"ShopAddress":addShopInfos[2],"ShopStates":addShopInfos[3]});
